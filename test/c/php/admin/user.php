...
function show_user_details($t)
{
    global $dbi;

    if (!$t->loadTemplateFile("admin-user-detail.tpl", true, true))
        return "<!-- ERROR: couldn't open admin-user-detail.tpl -->\n";
    ...
    if ($stmt->fetch())
    {
        ...
        $t->setVariable('STATUS', $r['status']);
        foreach (array('pending', 'active', 'locked') as $v)
        {
